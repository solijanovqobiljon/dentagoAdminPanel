...
const CoursesContent = () => {
    ...
    const [isEditModalOpen, setIsEditModalOpen] = useState(false); // Tahrirlash modali
    const [isPaymentModalOpen, setIsPaymentModalOpen] = useState(false);
    const [selectedCourse, setSelectedCourse] = useState(null);

    // Yangi kurs qo'shish uchun forma
    ...
    // Tahrirlash uchun alohida state
    ...
    // Rasm yuklash (qo'shish va tahrirlash uchun umumiy)
    ...
    const handleEditClick = (course) => {
        setEditingCourse({ ...course }); // Kursni nusxalab olamiz
        setIsEditModalOpen(true);
    };

    // Tahrirlangan ma'lumotlarni saqlash
    const handleSaveEdit = () => {
        if (!editingCourse.name || !editingCourse.teacher || !editingCourse.price) return;

        updateData('courses', editingCourse, 'UPDATE');
        ...
        <div className="p-4 md:p-8 space-y-8 bg-white dark:bg-slate-950 min-h-full">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {courses.length > 0 ? courses.map((course) => (
                    <div
                        key={course.id}
                        className="group flex flex-col bg-white dark:bg-slate-900 rounded-[2.5rem] overflow-hidden border border-blue-50 dark:border-blue-900/10 shadow-2xl shadow-blue-500/5 transition-all hover:translate-y-[-8px] hover:shadow-blue-500/10"
                    >
                        {/* Rasm qismi */}
                        <div className="relative h-56 overflow-hidden">
                            ...
                            <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-all">
                                <button
                                    ...
                                    className="p-2 bg-white/10 hover:bg-blue-500/80 text-white rounded-xl backdrop-blur-md transition-all"
                                    title="Tahrirlash"
                                >
                                ...
                                <button
                                    ...
                                    className="p-2 bg-white/10 hover:bg-red-500/80 text-white rounded-xl backdrop-blur-md transition-all"
                                    title="O'chirish"
                                >
                    ...
                    <div className="col-span-full py-20 flex flex-col items-center justify-center text-slate-400 space-y-4">
                        ...
                        <p className="text-lg font-bold italic">{t('no_courses') || 'Hozircha kurslar yo‘q'}</p>
                    </div>
                )}
            </div>
            ...
            <Modal
                ...
                title={t('edit_course') || "Kursni tahrirlash"}
                footer={
                    <button
